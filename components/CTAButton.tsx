...
export const CTAButton: React.FC<CTAButtonProps> = ({ children, size = 'normal', className, showIcon = false, ...props }) => {
    ...
    return (
        <a
            {...props}
            className={`inline-flex items-center justify-center font-sans font-bold text-white bg-brand-amber hover:bg-brand-amber-light rounded-lg shadow-xl shadow-black/20 transition-transform duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-amber-light w-auto whitespace-nowrap ${sizeClasses} ${className || ''}`}
        >
